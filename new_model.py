# Architecture of the novel GAN model
...
class Generator(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        # Encoder receives a noisy signal as input [BATCH_SIZE x 1 x 16384]
        self.enc1 = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=32, stride=2, padding=15, padding_mode = 'reflect')  # [BATCH_SIZE x 16 x 8192]
        self.enc1_nl = nn.PReLU()
        self.enc2 = nn.Conv1d(16, 32, 32, 2, 15, padding_mode = 'reflect')  # [BATCH_SIZE x 32 x 4096]
        self.enc2_nl = nn.PReLU()
        self.enc3 = nn.Conv1d(32, 32, 32, 2, 15, padding_mode = 'reflect')  # [BATCH_SIZE x 32 x 2048]
        self.enc3_nl = nn.PReLU()
        self.enc4 = nn.Conv1d(32, 64, 32, 2, 15, padding_mode = 'reflect')  # [BATCH_SIZE x 64 x 1024]
        self.enc4_nl = nn.PReLU()
        self.enc5 = nn.Conv1d(64, 64, 32, 2, 15, padding_mode = 'reflect')  # [BATCH_SIZE x 64 x 512]
        self.enc5_nl = nn.PReLU()
        self.enc6 = nn.Conv1d(64, 128, 32, 2, 15, padding_mode = 'reflect')  # [BATCH_SIZE x 128 x 256]
        self.enc6_nl = nn.PReLU()
        self.enc7 = nn.Conv1d(128, 128, 32, 2, 15, padding_mode = 'reflect')  # [BATCH_SIZE x 128 x 128]
        self.enc7_nl = nn.PReLU()
        self.enc8 = nn.Conv1d(128, 256, 32, 2, 15, padding_mode = 'reflect')  # [BATCH_SIZE x 256 x 64]
        self.enc8_nl = nn.PReLU()
        self.enc9 = nn.Conv1d(256, 256, 32, 2, 15, padding_mode = 'reflect')  # [BATCH_SIZE x 256 x 32]
        self.enc9_nl = nn.PReLU()
        self.enc10 = nn.Conv1d(256, 512, 32, 2, 15, padding_mode = 'reflect')  # [BATCH_SIZE x 512 x 16]
        self.enc10_nl = nn.PReLU()
        self.enc11 = nn.Conv1d(512, 1024, 32, 2, 15, padding_mode = 'reflect')  # [BATCH_SIZE x 1024 x 8]
        self.enc11_nl = nn.PReLU()

        # Decoder generates the enhanced speech
        # Decoder outputs are concatenated with homologous encoder outputs,
        # Thus, the feature map sizes are doubled
        self.dec10 = nn.ConvTranspose1d(in_channels=2048, out_channels=512, kernel_size=32, stride=2, padding=15)
        self.dec10_nl = nn.PReLU()  # out : [BATCH_SIZE x 512 x 16] -> (concat) [BATCH_SIZE x 1024 x 16]
        self.dec9 = nn.ConvTranspose1d(1024, 256, 32, 2, 15)  # [BATCH_SIZE x 256 x 32]
        self.dec9_nl = nn.PReLU()
        self.dec8 = nn.ConvTranspose1d(512, 256, 32, 2, 15)  # [BATCH_SIZE x 256 x 64]
        self.dec8_nl = nn.PReLU()
        self.dec7 = nn.ConvTranspose1d(512, 128, 32, 2, 15)  # [BATCH_SIZE x 128 x 128]
        self.dec7_nl = nn.PReLU()
        self.dec6 = nn.ConvTranspose1d(256, 128, 32, 2, 15)  # [BATCH_SIZE x 128 x 256]
        self.dec6_nl = nn.PReLU()
        self.dec5 = nn.ConvTranspose1d(256, 64, 32, 2, 15)  # [BATCH_SIZE x 64 x 512]
        self.dec5_nl = nn.PReLU()
        self.dec4 = nn.ConvTranspose1d(128, 64, 32, 2, 15)  # [BATCH_SIZE x 64 x 1024]
        self.dec4_nl = nn.PReLU()
        self.dec3 = nn.ConvTranspose1d(128, 32, 32, 2, 15)  # [BATCH_SIZE x 32 x 2048]
        self.dec3_nl = nn.PReLU()
        self.dec2 = nn.ConvTranspose1d(64, 32, 32, 2, 15)  # [BATCH_SIZE x 32 x 4096]
        self.dec2_nl = nn.PReLU()
        self.dec1 = nn.ConvTranspose1d(64, 16, 32, 2, 15)  # [BATCH_SIZE x 16 x 8192]
        self.dec1_nl = nn.PReLU()
        self.dec_final = nn.ConvTranspose1d(32, 1, 32, 2, 15)  # [BATCH_SIZE x 1 x 16384]
        self.dec_tanh = nn.Tanh()

        # Initializing weights weights
        self.init_weights()
    ...
    def forward(self, x, z):
        ...
        # Concatenating the thought vector with latent variable
...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        # D gets a combination of noisy signal and clear signal as input [BATCH_SIZE x 2 x 16384]
        negative_slope = 0.3
        self.conv1 = nn.Conv1d(in_channels=2, out_channels=32, kernel_size=31, stride=2, padding=15)  # [BATCH_SIZE x 32 x 8192]
        self.vbn1 = VirtualBatchNorm(32)
        self.lrelu1 = nn.LeakyReLU(negative_slope)
        self.conv2 = nn.Conv1d(32, 64, 31, 2, 15)  # [BATCH_SIZE x 64 x 4096]
        self.vbn2 = VirtualBatchNorm(64)
        self.lrelu2 = nn.LeakyReLU(negative_slope)
        self.conv3 = nn.Conv1d(64, 64, 31, 2, 15)  # [BATCH_SIZE x 64 x 2048]
        self.dropout1 = nn.Dropout()
        self.vbn3 = VirtualBatchNorm(64)
        self.lrelu3 = nn.LeakyReLU(negative_slope)
        self.conv4 = nn.Conv1d(64, 128, 31, 2, 15)  # [BATCH_SIZE x 128 x 1024]
        self.vbn4 = VirtualBatchNorm(128)
        self.lrelu4 = nn.LeakyReLU(negative_slope)
        self.conv5 = nn.Conv1d(128, 128, 31, 2, 15)  # [BATCH_SIZE x 128 x 512]
        self.vbn5 = VirtualBatchNorm(128)
        self.lrelu5 = nn.LeakyReLU(negative_slope)
        self.conv6 = nn.Conv1d(128, 256, 31, 2, 15)  # [BATCH_SIZE x 256 x 256]
        self.dropout2 = nn.Dropout()
        self.vbn6 = VirtualBatchNorm(256)
        self.lrelu6 = nn.LeakyReLU(negative_slope)
        self.conv7 = nn.Conv1d(256, 256, 31, 2, 15)  # [BATCH_SIZE x 256 x 128]
        self.vbn7 = VirtualBatchNorm(256)
        self.lrelu7 = nn.LeakyReLU(negative_slope)
        self.conv8 = nn.Conv1d(256, 512, 31, 2, 15)  # [BATCH_SIZE x 512 x 64]
        self.vbn8 = VirtualBatchNorm(512)
        self.lrelu8 = nn.LeakyReLU(negative_slope)
        self.conv9 = nn.Conv1d(512, 512, 31, 2, 15)  # [BATCH_SIZE x 512 x 32]
        self.dropout3 = nn.Dropout()
        self.vbn9 = VirtualBatchNorm(512)
        self.lrelu9 = nn.LeakyReLU(negative_slope)
        self.conv10 = nn.Conv1d(512, 1024, 31, 2, 15)  # [BATCH_SIZE x 1024 x 16]
        self.vbn10 = VirtualBatchNorm(1024)
        self.lrelu10 = nn.LeakyReLU(negative_slope)
        self.conv11 = nn.Conv1d(1024, 2048, 31, 2, 15)  # [BATCH_SIZE x 2048 x 8]
        self.vbn11 = VirtualBatchNorm(2048)
        self.lrelu11 = nn.LeakyReLU(negative_slope)
        self.convolutional_final = nn.Conv1d(2048, 1024, kernel_size=1, stride=1)  # [BATCH_SIZE x 1024 x 8]
        self.leakyrelu_final = nn.LeakyReLU(negative_slope)
        self.self_attn = SelfAttention(1024)
        self.convolutional_final2 = nn.Conv1d(1024, 512, kernel_size=1, stride=1) # [BATCH_SIZE x 512 x 8]
        self.leakyrelu_final2 = nn.LeakyReLU(negative_slope)
        self.self_attn2 = SelfAttention(512)        
        self.fully_connected = nn.Linear(in_features=8, out_features=1)  # [BATCH_SIZE x 1]
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        beta = nn.functional.softmax(torch.bmm(f.permute(0,2,1).contiguous(), g), dim=1)
